// pages/scale-syllabus.js (renamed)
...
export default function ScaleSyllabus() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
        <Link href="/ai-leadership-training" className="flex items-center text-mosaics-dark mb-8 hover:text-mosaics-purple">
          <ChevronLeft className="w-5 h-5 mr-2" /> Back to AI Leadership Training
        </Link>

        <h1 className="text-4xl font-bold mb-4 text-blue-900">AI Operational Leadership — Full 6-Week Curriculum</h1>
        <p className="text-gray-700 mb-8">
          Dive into each week’s agenda, pre-work, live exercises, and post-session deliverables—designed to install a
          repeatable operating rhythm for teams that want to move with clarity.
        </p>

        <Accordion type="multiple" collapsible className="space-y-4">
          {weeks.map((w, i) => (
            <AccordionItem key={i} value={`week-${i}`}>
              <AccordionTrigger className="text-xl font-bold text-blue-800">{w.title}</AccordionTrigger>
              <AccordionContent>
